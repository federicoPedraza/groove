...
import { CheckCircle2, LoaderCircle, SquareTerminal, Trash2, XCircle } from "lucide-react";
...
import { setIsCommandHistoryPanelOpen } from "@/src/lib/command-history-panel-state";
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        <span className={cn("size-1.5 shrink-0 rounded-full", presentation.dotColor)} />
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            ...
            >
              <IndicatorIcon
                className={cn("size-2.5", entry.state === "running" ? "animate-spin" : "")}
                aria-hidden="true"
              />
              {relativeTime}
            </span>
          ...
          {displaySublabel ? (
            <p className="mt-0.5 truncate text-[11px] text-muted-foreground/70">{displaySublabel}</p>
          ) : null}
        </div>
      </div>
...
export function CommandHistoryPanel() {
  ...
  return (
    <div ref={containerRef} className="fixed bottom-3 right-3 z-40">
      {isOpen ? (
        <section
          role="dialog"
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-border/50 px-3 py-2.5">
            <div className="flex items-center gap-2">
              <SquareTerminal className="size-3.5 text-muted-foreground" aria-hidden="true" />
              <span className="text-xs font-semibold text-foreground">Terminal Log</span>
              {hasEntries ? (
          ...
          {/* Command list */}
          <div ref={historyListRef} className="max-h-80 space-y-1 overflow-y-auto p-2" aria-live="polite">
            {!hasEntries ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <SquareTerminal className="mb-2 size-5 text-muted-foreground/30" aria-hidden="true" />
                <p className="text-[11px] text-muted-foreground/50">No commands yet</p>
              </div>
            ) : (
              completedEntries.map((entry) => (
                <CommandHistoryRow key={entry.id} entry={entry} now={now} mode={mode} />
              ))
            )}
          </div>
        </section>
      ...
      {/* Trigger: version + terminal icon */}
      <div className="flex items-center gap-1.5">
        <span className="select-none text-[11px] tabular-nums text-muted-foreground/50">v{APP_VERSION}</span>
        <button
          type="button"
          aria-label="Command history"

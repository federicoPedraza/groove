...
describe("useBarracksState", () => {
  ...
  it("hydrates active workspace state and recent directories from IPC response", async () => {
    ...
    const { result } = renderHook(() => useBarracksState());
  ...
  describe("notification listener integration", () => {
    ...
    it("sets viewing worktree when provided", async () => {
      ...
      const { result } = renderHook(() => useBarracksState("my-worktree"));
      await waitFor(() =>
        expect(result.current.isWorkspaceHydrating).toBe(false),
      );

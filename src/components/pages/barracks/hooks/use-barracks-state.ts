...
type BarracksWorkspaceSnapshot = {
...
const barracksWorkspaceSnapshot: BarracksWorkspaceSnapshot = {
...
export function useBarracksState(viewingWorktree?: string) {
  const [activeWorkspace, setActiveWorkspace] =
    useState<ActiveWorkspace | null>(
      () => barracksWorkspaceSnapshot.activeWorkspace,
    );
  ...
  useEffect(() => {
    barracksWorkspaceSnapshot.activeWorkspace = activeWorkspace;
  }, [activeWorkspace]);
  ...
  const fetchRuntimeState = useCallback(
    async (options?: { force?: boolean }): Promise<void> => {
      if (
        ...
        setActiveTerminalWorktrees(empty);
        publishActiveTerminalWorktrees(empty);
        return;
      }
      ...
      try {
        ...
        const next = new Set(result.worktrees);
        setActiveTerminalWorktrees(next);
  ...
  const closeCurrentWorkspace = useCallback(async (): Promise<void> => {
    try {
      ...
      setActiveTerminalWorktrees(emptyActive);
      clearWorkspaceContextStore();
      realtimeUnavailableRef.current = false;

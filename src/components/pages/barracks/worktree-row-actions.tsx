...
type WorktreeRowActionsProps = {
  ...
  variant?: "barracks" | "worktree-detail";
  ...
  onComment?: (worktree: string) => void;
  isCommentPending?: boolean;
  onViewComment?: (comment: CommentRecord) => void;
...
export function WorktreeRowActions({
  ...
  const commentActions =
    onComment || (latestComment && onViewComment) ? (
      <>
        {latestComment && onViewComment ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                type="button"
                ...
                className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground"
                onClick={() => {
                  onViewComment(latestComment);
                }}
                aria-label={`View commit comment for ${row.worktree}`}
              >
                <Sword aria-hidden="true" className="size-4" />
              </Button>
            </TooltipTrigger>
            <TooltipContent>View commit comment</TooltipContent>
          </Tooltip>
        ) : null}
        {onComment && !latestComment ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                type="button"
                ...
                className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground"
                onClick={() => {
                  onComment(row.worktree);
                }}
                disabled={isCommentPending || rowPending}
                aria-label={
                  isCommentPending
                    ? `Drafting commit comment for ${row.worktree}...`
                    : `Draft commit comment for ${row.worktree}`
                }
              >
                {isCommentPending ? (
                  <Loader2 aria-hidden="true" className="size-4 animate-spin" />
            ...
            <TooltipContent>
              {isCommentPending ? "Drafting comment..." : "Draft commit comment"}
            </TooltipContent>
          </Tooltip>
        ) : null}
      </>

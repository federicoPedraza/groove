    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between gap-2 text-xs">
        ...
        <span className="text-muted-foreground">
          {clampedValue == null ? "Unavailable" : formatPercent(clampedValue)}
        </span>
      </div>
...
function CollapsedMetricItem({
  label,
  value,
  labelClassName,
}: CollapsedMetricItemProps) {
  const clampedValue =
    typeof value === "number" ? Math.max(0, Math.min(100, value)) : null;
  ...
  return (
    <div className="px-2 py-1.5 text-center">
      ...
      <p className="text-xs font-medium text-foreground">
        {clampedValue == null ? "N/A" : formatIntegerPercent(clampedValue)}
      </p>
    </div>
  );
}

export function DiagnosticsSystemSidebar({
  collapsed,
  overview,
  isLoading,
  errorMessage,
  ...
  const cpuCoresLabel =
    overview?.cpuCores == null ? "Unavailable" : String(overview.cpuCores);
  const ramLabel =
      ...
      <SidebarContent className="space-y-3">
        {collapsed ? (
          <div className="space-y-2 pt-1">
            <CollapsedMetricItem
              label="CPU"
              value={overview?.cpuUsagePercent ?? null}
              labelClassName="text-amber-500"
            />
            <CollapsedMetricItem
              label="RAM"
              value={overview?.ramUsagePercent ?? null}
              labelClassName="text-blue-500"
            />
            <CollapsedMetricItem
              label="Swap"
              value={overview?.swapUsagePercent ?? null}
              labelClassName="text-rose-500"
            />
            <CollapsedMetricItem
              label="Disk"
              value={overview?.diskUsagePercent ?? null}
          ...
          <>
            <div className="space-y-2 rounded-md border bg-muted/20 p-2.5">
              <MetricRow
                ...
                trackClassName="bg-amber-500/20"
              />
              <MetricRow
                label="RAM"
                value={overview?.ramUsagePercent ?? null}
